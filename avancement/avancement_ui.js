...
init_avancement = function() {
   ...
   // Enregistre pour la première fois l'équation
    ...
    // Contenu html
    html = msg.AV_HTML_INTRO+"<div class='equation'>"
    ...
    html_table = "<table class='table'><thead class='thead-light'><tr>"
    ...
    for (var i = 0; i<current_equation.produits.length; i++){
...
let set_quantite = function(event){

    // met à jour les quantités avec les valeurs choisies
    let quantite = Array()
    if ($('input[name="choice_unites"]:checked').val() == 1){
        $("#av_get_quantite input[name=mol]").each(function(index){
            quantite.push(parseFloat($(this).val()))
        })   
    } else {
        $("#av_get_quantite input[name=gramme]").each(function(index){
            quantite.push(parseFloat($(this).val()))
        })
    }
...
let set_masses = function(data){
    ...
    let prods = current_equation.produits
...
let dsp_avancement = function(data){
    ...
    info = "<ul><li class='info'>" + txt.AV_TXT_LIMITANT + av.reactif_limitant+"</li>"
